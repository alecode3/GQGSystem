...
interface CuentasPagarTableProps {
  cuentas: CuentaPagarDetalle[];
  onPagarClick?: (cuentaId: number, monto: number) => Promise<void>;
}

export const CuentasPagarTable: React.FC<CuentasPagarTableProps> = ({
  cuentas,
  onPagarClick
}) => {
  ...
  const handlePagoSubmit = async (cuentaId: number, saldoMaximo: number) => {
    ...
    if (monto > saldoMaximo) {
      alert(`El monto a pagar no puede superar el saldo pendiente de ${formatCurrency(saldoMaximo, 'PYG')}.`);
      return;
    }
    ...
    if (onPagarClick) {
      ...
      try {
        await onPagarClick(cuentaId, monto);
        ...
        return (
          <tr key={cuenta.cuenta_id} className={`hover:bg-slate-50/50 transition-colors ${
            ...
            <td className="px-6 py-4 font-semibold text-emerald-600 text-right pr-6">
              {formatCurrency(cuenta.pagado, currencyAbrev)}
            </td>
                ...
                isEditingThis ? (
                  <div className="flex items-center gap-2 animate-fade-in">
                    ...
                    <Button
                      ...
                      onClick={() => handlePagoSubmit(cuenta.cuenta_id, cuenta.saldo)}
                    >
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      setPagoMonto(String(cuenta.saldo)); // Rellenar con saldo completo por defecto
                    }}
                  >
